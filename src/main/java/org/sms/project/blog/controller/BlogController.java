...
public class BlogController {
    ...
    public ResultAdd add(Model model, @Valid Blog blog, HttpServletRequest request) {
        ...
        return blogService.insert(blog, user, tags);
    }
    
    @ResponseBody
    @RequestMapping(value = "/delete", method = RequestMethod.POST)
    public ResultAdd delete(Model model, HttpServletRequest request) {
    ...
    public List<UploadFileBase> upload(@RequestParam("file") MultipartFile file, HttpServletRequest request) {
        User user = (User) SessionHelper.get(request, SysConstants.LOGIN_USER);
        UploadFileBase resAdd = blogService.addUpload(file, user);
    ...
    public String search(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (Objects.isNull(keyWord) || keyWord.trim() == "") {
            return null;
        }
        Integer pageNumber = Integer.parseInt(pageNumberStr);
        Page page = new Page(pageNumber, 10);
        String blogs = blogService.search(page, keyWord);
        return blogs;
    }
}